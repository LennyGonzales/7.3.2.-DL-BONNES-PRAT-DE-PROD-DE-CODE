...
export const gpsIntegrationTests = (
  suiteName: string,
  setup: () => GpsTestContext | Promise<GpsTestContext>
) => {
  describe(suiteName, () => {
    let context: GpsTestContext;
    ...
    it('addGps et verification avec listGpsByUserId', async () => {
      ...
      for (const gps of sample) {
        await context.service.createGps(gps);
      }

      const result = await context.service.listGpsByUserId(
    ...
    it('addGps et verification avec getGps', async () => {
      ...
      const created = await context.service.createGps(sample);
      const found = await context.service.getGps(created.id!);
    ...
    it('updateGps modifie un gps existant et verification avec getGps', async () => {
      ...
      const updatedGps = await context.service.updateGps(created.id!, updates);
    ...
    it('deleteGps et verification', async () => {
      ...
      const result = await context.service.deleteGps(created.id!);
